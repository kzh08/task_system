...
class Db
{
     ...
     * 构造函数-连接数据库
     *
     * @param bool|string $onlyRead 是否为只读库
     *
     * @throws DbException
     */
    public function __construct($dbConfigName = 'db', $onlyRead = false)
    {
        //如果为只读则从只读库列表读取一个库来读取
        if(!$onlyRead){
            $dbConfig = $GLOBALS['X_G'][$dbConfigName];
        }else{
            $dbConfig = $GLOBALS['X_G']["read_" . $dbConfigName];
            $len      = count($dbConfig);

            if($len > 1){
                $rand     = rand(0, $len - 1);
                $dbConfig = $dbConfig[$rand];
            }else{
                $dbConfig = $dbConfig[0];
            }
        }
    ...
    public function exec($sql)
    {
        ...
        if(count($GLOBALS['X_G']['backSql']) > 100){
            array_shift($GLOBALS['X_G']['backSql']);
        }

        if ($result = mysql_query($sql, $this->connection)) {
            return $result;
        } else {
            $mess = '数据库语句执行异常:' . $sql . ' error_log:' . mysql_error();
    ...
    public function insert($data)
    {
        ...
        foreach ($data as $key => $value) {
            ...
            $values .= empty($values) ? "'" . addslashes($value) . "'" : ",'" . addslashes($value) . "'";
        }

...
(function ($) {
	...
	function edittable(element, options){
		...
		this.changcell = function(o){
			...
			var val = strreplace(o.text()),
				nid = 'edittable_'+rand+'_'+timeci+'',
				off	= o.offset(),
				wid = o.width(),
				s	= '<div style="position: absolute;top:'+(off.top)+'px;z-index:8;left:'+off.left+'px;background:#ffffff;height:'+o.height()+'px;width:'+wid+'px">',
				canslw = 'fields="'+fid+'" id="'+nid+'"  changeid="'+js.changeid+'"';
				s='',
				othval = ntml.replace(val,'');
			if(xty == 'text'){
				s+= '<input style="width:'+(wid-10)+'px;line-height:18px;height:25px;text-align:center;border:1px #888888 solid" '+canslw+'></div>';
				o.html(s);
				var o1	= $('#'+nid+'');
				o1.focus();
				o1.val(val);
	...
	$.fn.edittable	= function(options){
		var defaultVal = {
			...
			data:{},
			keyfields:'id',//主键字段名
			saveurl:''+APP+'System/saveeditable/rnd/'+Math.random()+'',	//保存表格的地址
